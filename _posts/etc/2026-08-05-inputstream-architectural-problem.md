...
### Но не каждый `InputStream` владеет внешним ресурсом

Здесь важно сделать одно уточнение. Сам по себе тип `InputStream` еще не означает, что за ним обязательно стоит файл, сокет или другой внешний ресурс.
...
В данном случае поток читает данные из обычного массива байтов, уже находящегося в памяти JVM. Он не открывает файл, не удерживает файловый дескриптор и не использует сетевое соединение.

Поэтому вызов `close()` у `ByteArrayInputStream` фактически ничего не освобождает. Более того, согласно его контракту, закрытие не оказывает на поток никакого эффекта: после вызова `close()` его по-прежнему можно использовать.

То же самое относится к `ByteArrayOutputStream`:

```java
ByteArrayOutputStream output = new ByteArrayOutputStream();
```

Он накапливает данные во внутреннем массиве байтов и также не владеет внешним ресурсом. Его `close()` не требует освобождения каких-либо ресурсов. Это хорошо видно, например, в небольших тестах:
...
Оборачивать такие потоки в `try-with-resources` технически можно, но с точки зрения управления ресурсами это ничего не дает. Поэтому распространенное правило «любой `InputStream` необходимо закрывать» стоит воспринимать как полезное практическое упрощение, а не как фундаментальное свойство самого `InputStream`.

Гораздо точнее сформулировать вопрос так:

> **Какой конкретно источник находится за этим потоком и кто отвечает за его жизненный цикл?**

Для `FileInputStream` таким источником будет открытый файл. Для потока сокета — сетевое соединение. Для `ByteArrayInputStream` — обычный массив в памяти, не требующий явного освобождения.

При этом архитектурные особенности `InputStream` никуда не исчезают даже в последнем случае. `ByteArrayInputStream` все равно остается **состоянием процесса чтения**: у него есть текущая позиция, его можно прочитать до конца, а результат последующих операций зависит от того, что с ним происходило раньше.

Таким образом, у `InputStream` полезно различать две независимые характеристики:
...
InputStream
    ...
    │      ├── текущая позиция
    │      ├── EOF
    │      └── история предыдущих чтений
    │
    └── владение внешним ресурсом
           ├── файл
           ├── сокет
           ├── соединение
           └── может отсутствовать
...
Именно первая характеристика присуща потоковой абстракции как таковой. Вторая зависит уже от конкретной реализации.
...
С `InputStream` ситуация сложнее. Многие его реализации владеют внешним ресурсом или управляют доступом к нему:
