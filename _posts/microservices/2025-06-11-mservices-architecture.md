...
> ⚠️ Микросервис — это архитектурное решение, где каждый компонент системы — это отдельное Spring Boot приложение (или сервис на 
> любом другом языке). Spring Cloud придает этой системе "связность" и возможности распределенных приложений.
...
| ---------------------- | --------------------------------------------------- | --------------------------------------------------------------------------- |
| **Layered per entity** | CRUD-приложения, админ-панели,<br>rapid prototyping | Быстрая генерация.<br>Минимальная предметная логика.                        |
| **Use-case oriented**  | Производственные системы,<br>DDD-проекты            | Сервис отражает бизнес-операцию.<br>Высокая связность к предметной области. |
